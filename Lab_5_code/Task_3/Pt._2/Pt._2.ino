/* Purpose: to create a compass reading based on magnetic field values
    Stephen Shaw
    4 May 2020
*/
...
void loop()
{
  ...
  float declination = 0.037; // in radians
  float heading = atan2(mag_x, mag_y);
  heading += declination;
  if (heading < 0)
  {
    heading += 2 * PI; //correcting signs from [-180,180] to [0, 360]
  }
  if (heading > 2 * PI)
  {
    heading -= 2 * PI; //due to added declination
  }
  float headingDegrees = (heading * 180) / (2 * PI); //convert heading to degrees
  lcd.begin(16, 2);
  lcd.clear();
  lcd.setCursor(0, 0);
  lcd.print("Compass Degrees");
  lcd.setCursor(0, 1);
